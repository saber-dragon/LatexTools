...
# hacked by Long Gong
_input_commands = ["input", "include", "subfile", "loadglsentries", "externaldocument"]
_import_commands = [
...
def _analyze_tex_file(tex_root, file_name=None, process_file_stack=[],
                      ana=None, import_path=None, options=None):
    ...
    for m in _RE_COMMAND.finditer(content):
        ...
        if g("command") == "label" and g("args") is not None and options is not None:
            entryDict.update(
                {"args": options + g("args")}
            )
        ...
        print("Handle input/import commands")
        if g("command") in _input_commands and g("args") is not None:
            print("{}: {}".format(g("command"), g("args")))
            process_file_stack.append(file_name)
            if g("command") == "externaldocument":
                full_file = os.path.abspath(os.path.join(base_path, g("args")))
                print("full path: ", full_file)
                external_path = os.path.dirname(full_file)
                process_file_stack.append(file_name)
                open_file = full_file
                _analyze_tex_file(tex_root, open_file, process_file_stack, ana, import_path=external_path, options=g("optargs"))
            else:
                if import_path is not None:
                    # next_import_path = os.path.join(import_path, g("args"))
                    # next_import_path = os.path.normpath(next_import_path)
                    open_file = os.path.join(import_path, g("args"))
                    _analyze_tex_file(tex_root, open_file, process_file_stack, ana, import_path=import_path, options=options)
                else:
